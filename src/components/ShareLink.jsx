...
export default function ShareLink(props) {
  ...
  const updateGistLink = (event) => {
    ...
    if (gistId) {
      setGistLink(`https://fshonline.chrism.org/#/gist/${gistId}`);
    }
  };
  ...
  const handleOpenShare = async () => {
    ...
    const longLink = `https://fshonline.chrism.org/#/share/${encoded}`;
    ...
    if (bitlyLink.errorNeeded === true) {
      ...
      const displayLink = `https://fshonline.chrism.org/#/share/${bitlySlice}`;
      setLink(displayLink);
      setOpenShare(true);
      setCopyTip('Copy to Clipboard');
